...
$guards | %{$_ | add-member -name "Sleep" -MemberType NoteProperty -value 0}
$guards | %{$_.sleep = (($_.group | measure delta -sum).sum)}
...
$guards | %{$_ | add-member -MemberType noteproperty -Name SleepMinCnt -Value $null}

$guards | %{$_.sleepmin = 0}
$guards | %{$_.sleepminCnt = 0}
foreach ($guard in $guards) {
    $max = ($guard.sleepsched | sort sleepcount -desc | select -first 1)
    $guard.sleepmin = $max.Min
    $guard.sleepmincnt = $max.sleepcount
}

$guards | sort sleepmincnt -desc | select -first 1 | ft name,sleep,sleepmin,@{Label="Mult";Expression={[int64]$_.name * $_.SleepMin} }
